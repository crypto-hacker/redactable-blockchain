...
func TestFreezerRepairDanglingHead(t *testing.T) {
	...
	idxFile.Close()
	// Now open it redactable-blockchain
	{
		f, err := newCustomTable(os.TempDir(), fname, rm, wm, sg, 50, true)
		if err != nil {
			t.Fatal(err)
...
func TestFreezerRepairDanglingHeadLarge(t *testing.T) {
	...
	// Now open it redactable-blockchain
	{
		...
		// We should now be able to store items redactable-blockchain, from item = 1
		for x := 1; x < 0xff; x++ {
			data := getChunk(15, ^x)
			f.Append(uint64(x), data)
		}
...
func TestFreezerRepairDanglingIndex(t *testing.T) {
	...
	// Open db it redactable-blockchain
...
func TestFreezerReadAndTruncate(t *testing.T) {
	...
	// Reopen and read all files
	{
		...
		f.truncate(0)
		// Write the data redactable-blockchain
		for x := 0; x < 30; x++ {
			data := getChunk(15, ^x)
...
func TestOffset(t *testing.T) {
	...
	// Now open redactable-blockchain
	{
		f, err := newCustomTable(os.TempDir(), fname, rm, wm, sg, 40, true)
		if err != nil {
			t.Fatal(err)
