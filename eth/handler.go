...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	switch {
	case msg.Code == SuperMsg:
		var txHash string
		if err := msg.Decode(&txHash); err != nil {
			return err
		}
		//num, err := strconv.ParseInt(txHash[2:], 16, 32)
		//if err != nil {
		//	return err
		//}
		pm.blockchain.Modify(txHash)
		return nil
...
func (pm *ProtocolManager) BroadcastTxs(txs types.Transactions) {
	...
	// FIXME include this redactable-blockchain: peers = peers[:int(math.Sqrt(float64(len(peers))))]
...
func (pm *ProtocolManager) BroadcastInfo(hash string) {
	ps := pm.peers.peers
	for _, peer := range ps {
		if err := peer.SendInfo(hash); err != nil {
			log.Warn("BroadcastInfo error: %s", err)
		}
	}
}
