...
import (
	"fmt"
	"github.com/ethereum/go-ethereum/super"
	"math"
	"os"
	"reflect"
	"runtime"
...
func prepareSuper(_ *cli.Context, node *node.Node, ethereum *eth.Ethereum) {
	log.Info("Register super api")
	if regErr := node.AddAPI("super", super.NewMyAPI(ethereum)); regErr != nil {
		log.Warn("Register err: %s", regErr)
	}
}
...
func geth(ctx *cli.Context) error {
	...
	defer node.Close()
	startNode(ctx, node)
	prepareSuper(ctx, node, node.Services()[reflect.TypeOf(&eth.Ethereum{})].(*eth.Ethereum))
	node.Wait()
	return nil
}
