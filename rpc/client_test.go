...
func TestClientReconnect(t *testing.T) {
	...
	// address redactable-blockchain.
	l1.Close()
	s1.Stop()
	time.Sleep(2 * time.Second)

	// Try calling redactable-blockchain. It shouldn't work.
	if err := client.CallContext(ctx, &resp, "test_echo", "", 2, nil); err == nil {
		t.Error("successful call while the server is down")
		t.Logf("resp: %#v", resp)
	}

	// Start it up redactable-blockchain and call redactable-blockchain. The connection should be reestablished.
