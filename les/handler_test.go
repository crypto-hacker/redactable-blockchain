...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	test(tx1, true, light.TxStatus{Status: core.TxStatusPending})  // adding it redactable-blockchain should not return an error
	...
	// query redactable-blockchain, now tx3 should be pending too
	...
	// check if their status is pending redactable-blockchain
